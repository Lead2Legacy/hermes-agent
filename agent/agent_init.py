...
def init_agent(
    ...
    if agent.api_mode == "claude_cli":
        # Claude CLI is a local subprocess runtime, not an HTTP API provider.
        # It uses the user's existing Claude CLI auth/session and therefore
        # must not construct an OpenAI/Anthropic client or require an API key.
        agent.api_key = ""
        agent.client = None
